...
|--------|---------|--------------|---------|
| `cfs` | | [`/printer/objects/query`](../metrics/cfs) | 15 |
...
Creality Filament System (CFS) metrics for K2-class printers (native `box` and
`filament_rack` objects). Use this instead of `mmu` on Creality hardware.
15 metrics covering:
